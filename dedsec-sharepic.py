...
def webhook(file_contents, extension):
    webhook = DiscordWebhook(url=f'{webhook_url}', username="dump pictures")
    rands = ''.join(random.choices('qwertyuioplkjhgfdsazxcvbnm', k=10))
    with open(f"{file_contents}".rstrip('\n'), "rb") as f:
        webhook.add_file(file=f.read(), filename=f'{rands}.{extension}')
        webhook.execute()
